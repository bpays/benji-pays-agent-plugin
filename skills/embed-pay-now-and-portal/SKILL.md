...
Copy the stable link from Benji and replace its invoice number and total placeholders with that system's template tokens. Official support has first-class guides for HaloPSA, ConnectWise PSA, Datto Autotask, Quoter, and QuoteWerks; the generic flow works for other systems that sync the invoice/customer into supported accounting.
...
- HaloPSA: https://support.benjipays.com/support/solutions/articles/150000211767-integrating-benji-pays-pay-now-link-with-halopsa-invoices
...
Inspect `data.customerPortal` for `portalName` and controls such as `payNowAmountRequired`, `disableGenericPaymentLink`, and portal behavior. Do not guess a merchant's portal name or gateway/currency template.
